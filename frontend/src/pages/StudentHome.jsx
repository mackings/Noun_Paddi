...
// Projects, Plagiarism, then the rest. Tab-bar destinations (Courses, Quiz, Ask) are also
// cross-listed here for first-run discoverability, since a first-time student sees this
// grid before learning the bottom tab bar exists. Profile is not listed here — it's the
// top-right icon button instead, since it's account management, not a study feature.
// "TMA" has no dedicated page yet, so it routes into Ask (Past Questions) alongside the
// existing Past Questions tile, since Ask already fields TMA-related questions.
const FEATURES = [
  ...
  { key: 'tma', path: '/ask', label: 'TMA', description: 'Get help with your TMA', icon: ClipboardList },
...
const StudentHome = () => {
  ...
  return (
    <div className="tw:space-y-5 tw:p-5">
      <div className="tw:flex tw:items-start tw:justify-between tw:gap-3">
        ...
        <Link
          to="/profile"
          aria-label="Profile"
          className="tw:flex tw:h-10 tw:w-10 tw:flex-none tw:items-center tw:justify-center tw:rounded-full tw:bg-brand-100 tw:text-brand-600 tw:dark:bg-brand-950 tw:dark:text-brand-300"
        >
      ...
      <div className="tw:grid tw:grid-cols-2 tw:gap-3">
        {FEATURES.map(({ key, path, label, description, icon: Icon }) => (
